...
let
  ...
  isBuildableOnHost =
    homeConfig: homeConfig.pkgs.stdenv.hostPlatform.system == pkgs.stdenv.hostPlatform.system;

  # isTestableOnHost :: NixosConfig -> Bool
  isTestableOnHost = _: homeConfig: hasTestableSystem homeConfig && isBuildableOnHost homeConfig;

  # mkConfigListItem :: HomeConfig -> AttrSet
  mkConfigListItem =
    name: homeConfig: lib.nameValuePair "host-test-${name}" homeConfig.activationPackage;
in
