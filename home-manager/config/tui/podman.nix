{ config, pkgs, ... }: {
  ...
  xdg.configFile."containers/containers.conf".source = (pkgs.formats.toml { }).generate "containers.conf" {
    containers = {
      default_sysctls = [
  ...
  xdg.configFile."containers/seccomp.json".source = (pkgs.formats.json { }).generate "seccomp.json" {
